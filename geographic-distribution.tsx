...
const GeographicDistribution = () => {
  ...
  const [missingGeoInfo, setMissingGeoInfo] = useState(0);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await window.fs.readFile('UCLA Comparative Catalog updated locality info.xlsx  Combined colls 5 26 23.csv');
        ...
        Papa.parse(text, {
          ...
          complete: (results) => {
            ...
            setTotalSpecimens(results.data.length);
            setLoading(false);
          },
          error: (error) => {
